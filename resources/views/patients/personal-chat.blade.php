...
<div class="container-fluid p-4" style="background-color: white;">
    ...
    <div class="container">
        <div class="row">
            ...
            <h3><a href="/patient/chat"><span class="la la-arrow-left"></span></a>
            <span class="mb-0 ml-3">{{$hw->name}} {{$hw->surname}}</span></h3>
            <span class="text-secondary ml-5 small"> View and chat with <span class="font-weight-bold">{{$hw->cadre}}</span> {{$hw->name}} </span>  
            </div>
    ...
    <br>
     ...
     <div class="container p-3">
        <form method="POST" action="{{ route('patient-send-chat') }}">
            @csrf
             <!--send the hw's id--> 
             <input type="hidden" name="h_id" value="{{$hw->id}}"/>
